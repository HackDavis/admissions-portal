...
describe('reserveMailchimpAPIKeyIndices', () => {
...
describe('incrementMailchimpApiKeyIndex', () => {
  ...
  test('throws if incrementing beyond maxApiKeys', async () => {
    mockedGetMailchimp.mockResolvedValue({
      ok: true,
      body: { apiKeyIndex: 5, maxApiKeys: 5 },
    });
  ...
  test('handles getMailchimp failure', async () => {
    mockedGetMailchimp.mockResolvedValue({
      ok: false,
